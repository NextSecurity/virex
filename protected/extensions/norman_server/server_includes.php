...
define('HASH_SUPPORTED_SHA256', "false");
...
class ServerObject {
    ...
    function virex_init() {
        ...
        if ((!$this->virex_currentUser->rights_daily) || (!$this->virex_currentUser->rights_monthly)) {
            if ($this->virex_currentUser->rights_monthly) {
                $this->virex_ExtraConditions['clean'] .= ' AND type_scl = "M"';
                $this->virex_ExtraConditions['detected'] .= ' AND type_sde = "M"';
            } elseif ($this->virex_currentUser->rights_daily) {
                $this->virex_ExtraConditions['clean'] .= ' AND type_scl = "D"';
                $this->virex_ExtraConditions['detected'] .= ' AND type_sde = "D"';
            } else {
        ...
        if (!$this->virex_currentUser->rights_clean) {
            $this->virex_ExtraConditions['clean'] = "1=2 ";
        }
    ...
    public function get_sample($hash) {
        ...
        if ($this->cleanfile) {
            $res = $this->sql->query("SELECT hex(md5_scl) as hash, md5_scl 'md5', file_size_scl 'fsize' FROM $table WHERE {$hash_type}_scl='$hash' AND " . $this->virex_ExtraConditions['clean']);
        } else {
            $res = $this->sql->query("SELECT hex(md5_sde) as hash, md5_sde 'md5', file_size_sde 'fsize' FROM $table WHERE {$hash_type}_sde='$hash' AND " . $this->virex_ExtraConditions['detected']);
        }
