...
class SamplesCommand extends ConsoleCommand {
     ...
     * @param <type> $nomail - for debug only.. to show message and no send
     */
    public function actionWeeklystats($nomail = false) {
        ...
        $content .= "Errors in uploads: " . $this->pad($bogus['nr']) . "\r\n";
    ...
    private function size($size) { // shortcut to FileHelper static method formatSize()
        return $this->pad(FileHelper::formatSize($size));
    }

    private function pad($input) {
        return $input;
//        return str_pad($input, 12, ' ', STR_PAD_LEFT);
    }
