...
class BogusController extends Controller {
        ...
        if (isset($_GET['action'])) {
            ...
            switch ($_GET['action']) {
                ...
                case 'unpack':
                    ...
                    Yii::app()->user->setFlash('_success', "Archive file will be reprocessed soon!");
                    header("Location: " . $_SERVER['HTTP_REFERER']);
                    break;
                case 'delete':
    ...
    public function actionBogus_mass() {
        ...
        if (isset($_POST['extendedSelect'])) {
            ...
            if ($action == 'unpack') {
                Yii::app()->db->createCommand("UPDATE bogus_archives_bga SET pending_action_bga='Rescan' WHERE {$criteria->condition}")->execute($criteria->params);
            ...
            $number = $number['nr'];
            if ($action == 'unpack') {
                Yii::app()->db->createCommand("UPDATE bogus_archives_bga SET pending_action_bga='Rescan' WHERE {$condition}")->execute();
                Yii::app()->user->setFlash('_success', $number . ' archives will be reprocessed!');
            } elseif ($action == 'delete') {
