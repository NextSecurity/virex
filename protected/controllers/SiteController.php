...
class SiteController extends Controller {
    ...
    public function actionSearch_file() {
        ...
        if (isset($_POST['search_md5'])) {
            ...
            $file = Yii::app()->db->createCommand()->select('md5_sde, hex(md5_sde) "hex"')->from('samples_detected_sde')->where('md5_sde =:md5 AND added_when_sde>="' . $min_date . '" AND enabled_sde = 1 ' . $rightsCondition, array(':md5' => $_POST['search_md5']))
                    ->queryRow();
            if ($file) {
                header("Content-type: application/octet-stream");
            ...
            $files = Yii::app()->db->createCommand()->select('md5_sde, file_size_sde')->from('samples_detected_sde')
                            ->where('added_when_sde>="' . $min_date . '" ' . $rightsCondition . ' and enabled_sde=1 AND detection_sde LIKE :detection', array(':detection' => $search . '%'))->group('md5_sde')->limit(1000)->queryAll();
            $cont = '';
            $f_id = 0;
