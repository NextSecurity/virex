...
class AFileArchive {
    ...
    private function save_to_db($detection, $type, $md5, $size) {
        switch ($detection) {
            case 'detected':
                $q = "INSERT INTO samples_detected_sde (added_when_sde, md5_sde, file_size_sde, type_sde) VALUES (NOW(), :md5, :size, :type) ON DUPLICATE KEY UPDATE added_when_sde = NOW()";
                break;
            case 'clean':
                $q = "INSERT INTO samples_clean_scl (added_when_scl, md5_scl, file_size_scl, type_scl) VALUES (NOW(), :md5, :size, :type) ON DUPLICATE KEY UPDATE added_when_scl = NOW()";
                break;
        }
        Yii::app()->db->createCommand($q)->execute(array(':md5' => $md5, ':size' => $size, ':type' => $type));
    ...
    private function ascii2hex($ascii) { // for sampleshare framework
        $hex = '';
    ...
    private function makedir($file, $baseUrl) { // for sampleshare framework
