...
class AUrlArchive {
    ...
    public function read_file($file, $mime) {
        ...
        $f = fopen($file, 'r');
        $insert = Yii::app()->db->createCommand("INSERT INTO urls_url (md5_url, url_url, added_when_url) VALUES	(md5(:url), :url, CURDATE()) ON DUPLICATE KEY UPDATE added_when_url = CURDATE()");
        while ($line = fgets($f)) {
            if (trim($line) && $this->valid_url(trim($line))) {
                $insert->execute(array(':url' => trim($line)));
            }
        }
        unset($insert);
        fclose($f);
